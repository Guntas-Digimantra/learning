...
const FeatureItem = memo(({ item }: { item: (typeof feature_data)[0] }) => {
  return (
    <div className="w-1/4 flex-[0_0_auto] px-[15px] max-[991px]:w-1/2 max-[767px]:w-full">
      <div className="mb-[30px] text-center">
        <div className="mb-[22px] min-h-[93px] transition-all duration-300 ease-out">
          <Image src={item.icon} width={100} height={100} alt="img" loading="lazy" />
        </div>
        <div className="text-white">
          <h4 className="mb-2 text-xl font-medium">{item.title}</h4>
          <p className="text-base text-[#acaacc]">{item.desc}</p>
        </div>
      </div>
    </div>
  );
...
const Features = () => {
  return (
    <section className="bg-[#282568] px-0 pb-[90px] pt-[120px] max-[767px]:py-[60px]">
      <div className="mx-auto max-w-[1440px] px-[15px]">
        <div>
          <div className="mx-auto w-1/2 pb-10 text-center text-white max-[991px]:w-full">
            <h2 className="text-[36px] text-white max-[767px]:text-[28px]">
              Start Your Learning Journey Today!
            </h2>
            <p className="text-base text-[#acaacc]">
