...
async function shot(url, v2) {
  ...
  await page.locator('.blog-post-content').scrollIntoViewIfNeeded();
  await page.waitForTimeout(800);
  return page.locator('.blog-post-content').screenshot();
}
...
console.log(`${parseFloat(pct) <= 2 ? 'PASS' : 'FAIL'}  blog-post-content: ${pct}%  ref ${ref.width}x${ref.height}  v2 ${v2.width}x${v2.height}`);
await browser.close();
