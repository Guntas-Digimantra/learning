...
async function shot(url, v2) {
  ...
  const buf = await page.locator('header .dropdown-content').first().screenshot();
  await page.close();
  return buf;
}

const refBuf = await shot(REF, false);
const v2Buf = await shot(V2, true);
...
console.log(`${parseFloat(pct) <= 2 ? 'PASS' : 'FAIL'}  dropdown: ${pct}%  ${w}x${h}`);
await browser.close();
